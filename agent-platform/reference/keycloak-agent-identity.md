...
- **과잉 권한**: "조회만 하는 봇"이라며 만든 계정이 실제로는 그 API 의 전 범위를 갖는다.
...
**client 는 사람이 만들지 않는다.** 승격 게이트 CI 의 `sync_keycloak_client.py` 가
`agent.yaml` 의 `tools[].scope` 를 읽어 재생성한다. 코드가 선언보다 넓은 권한을 쓰려 해도
토큰에 스코프가 없어 API 에서 거부된다. **선언과 실권한이 구조적으로 일치한다.**
...
sequenceDiagram
    ...
    Note over U,A: 에이전트는 사용자 access token 을 보관한다<br/>(짧은 수명, 재발급은 refresh token)

    U->>A: "어제 천안 반품 건수?"
    A->>GW: tools/call wms.query<br/>Authorization: Bearer <사용자 토큰>

    GW->>GW: ① 이 에이전트가 wms.query 를 쓸 수 있는가<br/>(등록부 tools allowlist)
    ...
    KC->>KC: agent-cs-triage 가 이 교환을 할 수 있는가<br/>+ 사용자가 read:inventory 를 가졌는가
...
핵심은 4~5단계다. **에이전트는 자기 권한으로 API 를 부르지 않는다.**
...
4. 필요한 scope 를 client scope 로 생성: `read:inventory`, `write:refund` …
...
`agent-*` client 와 `wms-api` 쪽에도 만들어야 교환 토큰에 클레임이 실린다.
...
### 4-3. 에이전트 client 생성 (CI 가 수행)

1. Clients → Create client → `agent-cs-triage`
2. Client authentication **On** (confidential), 시크릿은 Vault 로만
3. **Standard token exchange** 활성화
   - Keycloak 26.x 는 표준 토큰 교환(RFC 8693)을 클라이언트별 토글로 제공한다.
   - 레거시 내부 교환과 별개 기능이며, 서버 기동 옵션/기능 플래그가 필요할 수 있다.
   - **현재 사내 버전에서 어느 쪽이 켜져 있는지 먼저 확인할 것.**
4. Client scopes 에 이 에이전트가 쓸 scope 만 Optional 로 부착
   - `agent.yaml` 의 `tools[].scope` 와 정확히 일치해야 한다 (CI 가 대조)
...
| service account 는 **L2 승인 + 최소 role** | 사용자 권한 상속이 불가능한 경우의 예외 |
| 교환 토큰 수명 **15분 이하** | 유출 창 최소화 |
| scope 는 **Optional** 로만 부착 | Default 로 두면 항상 실려서 최소권한이 깨진다 |
| API 쪽에서 **audience + scope 를 반드시 검증** | 토큰만 발급하고 API 가 검증 안 하면 전부 무의미 |
...
2. 각 client 의 scope vs `agent.yaml` 선언 — 콘솔에서 수동으로 넓힌 흔적
3. service account 를 쓰는 에이전트 전수 — L2 승인 이력이 있는가
